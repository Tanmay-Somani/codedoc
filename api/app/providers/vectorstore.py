...
class QdrantStore:
    ...
    def __init__(self, settings: Settings) -> None:
        self._client = AsyncQdrantClient(url=f"http://{settings.qdrant_host}:{settings.qdrant_port}")
    ...
    async def search(self, collection: str, vector: list[float], *, limit: int = 10) -> list[dict[str, Any]]:
        if not await self._collection_exists(collection):
            return []
        hits = await self._client.search(collection_name=collection, query_vector=vector, limit=limit)
