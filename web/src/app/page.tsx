...
function StatCard({
  ...
  return (
    ...
    >
      <Card className="group cursor-default hover:border-primary/30">
  ...
  return (
    ...
    >
              ...
              <div className="space-y-2 text-sm">
                {integrations.data ? (
                  Object.entries(integrations.data.providers).map(([key, name]) => (
                    <div
                      key={key}
                      className="flex items-center justify-between rounded-lg px-2 py-1.5 transition-colors duration-200 hover:bg-muted/40"
                    >
      ...
      <motion.div
        ...
        animate="show"
      >
        {(Object.keys(severityStyles) as Severity[]).map((sev, i) => (
          <motion.div key={sev} variants={item} transition={{ delay: 0.5 + i * 0.05 }}>
            <Card className="flex items-center justify-between p-4 hover:border-primary/30 transition-colors duration-200">
              <span className="flex items-center gap-2 text-sm capitalize font-medium">
                <ShieldAlert className="h-4 w-4 text-muted-foreground" />
                {sev}
              </span>
              <Badge className={severityStyles[sev]}>0</Badge>
            </Card>
          </motion.div>
        ))}
      </motion.div>
